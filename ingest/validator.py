...
try:
    ...
    import subprocess
    subprocess.check_call([sys.executable, "-m", "pip", "install", "openai", "pydantic"])
    import openai
...
# =====================================================================
# 2. 托管大模型调用客户端（通过 LiteLLM 统一代理）
# =====================================================================

class Processor:
    """标准规范化的 LLM 评估代理客户端管理器"""
    def __init__(self, prompt_hub_path: str = "prompt_hub.yaml", cuda_device: str = "0", litellm_base_url: str = "http://172.17.0.1:4000/v1"):
        self.factory = ModelFactory(prompt_hub_path=prompt_hub_path)
        self.litellm_base_url = litellm_base_url
        
        # 初始化 OpenAI 兼容客户端，统一指向 LiteLLM 代理
        self.client = openai.OpenAI(
            api_key="sk-1234",  # LiteLLM 默认代理密钥
            base_url=self.litellm_base_url
        )
        logging.info(f"✅ LiteLLM 智评客户端初始化成功，代理地址: {self.litellm_base_url}")
...
class RAGDataValidator:
    ...
    def _evaluate_text_via_llm(self, context_summary: str, chunk_texts: List[str], llm_model: str) -> float:
        """通过 LiteLLM 代理调用后端模型对抽样文本进行综合评分 (0-20分)"""
        if not self.processor:
            self.logger.warning("⚠️ 未注入 Processor，跳过 LLM 评估维度（默认给予基础分 15 分）")
            return 15.0

        try:
            client = self.processor.get_client()
            sampled_chunks_str = "\n".join([f"[分块 {i+1}]: {text}" for i, text in enumerate(chunk_texts)])
            
            messages = [
                {
                    "role": "system",
                    "content": "你是一个冷酷、严厉的 RAG 数据质量评估机器。你不需要具备人类的社交礼仪，不需要解释，只能输出数字。"
                },
                {
                    "role": "user",
                    "content": (
                        ...
                        f"如果你输出了除数字外的任何字符，系统将会崩溃。请直接给出最终的浮点数："
                    )
                }
            ]

            self.logger.info(f"🤖 正在通过 LiteLLM 代理请求模型 {llm_model} 进行语义评估...")
            response = client.chat.completions.create(
                model=llm_model,  # 对应 LiteLLM 中配置的 model_name，如 qwen3-4b
                messages=messages,
                max_tokens=8,
                temperature=0.1
            )
            ...
            match = re.search(r"\d+\.\d+|\d+", raw_response_text)
            if match:
            ...
            self.logger.info(f"🤖 LLM 语义维度的打分结果为: {score} / 20.0 (原始内容: {raw_response_text})")
            return round(score, 1)
            
        except Exception as e:
            self.logger.error(f"❌ 通过 LiteLLM 进行质量评估推理失败: {e}，启用安全降级策略分配 12.0 分")
            return 12.0
    ...
    def validate_json_file(self, file_path: str, llm_model: str) -> Tuple[bool, float, str]:
        ...
        # 2. 多维度软性打分（包含通过 LiteLLM 调用的 LLM 打分）

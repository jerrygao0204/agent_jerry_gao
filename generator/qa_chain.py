...
#         # 1. 获取双工厂单例句柄
#         self.vllm_factory = ModelFactory()

#         # 挂载通用 Prompts
#         self.prompts = self.vllm_factory.prompts

#         # 初始化 API/vLLM LLM 客户端（无需直接占用显存装载静态原生 PyTorch 模型）
...
#             "rag_qa",
#             "你是一个专业的 RAG 助手。请严格依据以下提供的参考资料回答用户的问题。\n"
...
#         # 如果需要应用模板，可借由 hf_factory 内部的 Tokenizer 执行，或直接交给 llm_client 消费 messages
...
#             )
#         # 若未直接加载纯文本 Tokenizer，将组装好的 messages 结构传给 llm_client 内部解析
#         return user_content
...
#         for response in self.llm_client.stream_generate(query=full_prompt, model_name="qwen3-4b"):
#             # 获取文本内容
...
#     test_query = "请问怎么清理 FineBI 的日志文件？"

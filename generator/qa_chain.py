...
# class QAChain:
#     """
#     RAG 核心问答链（端到端：混合检索 -> 交叉重排 -> Prompt拼装 -> 流式生成）
#     """
#     def __init__(
#         self,
#         cuda_device: str = "0",
...
#         top_k_rerank: int = 3,
#         llm_short_name: str = "Qwen/Qwen3-32B"
#     ):
...
#         logging.info("✅ QAChain 全链路初始化完毕，随时准备接收提问！")
...
#                       history: Optional[List[Dict[str, str]]] = None, # 🌟 补齐 history 参数
#                       filter_expr: Optional[str] = None, 
#                       pre_retrieved_chunks: Optional[List[Dict[str, Any]]] = None) -> Generator[str, None, None]:
#         """
#         流式问答生成入口
#         :param query: 用户提问
#         :param history: 对话历史 [{"user": "...", "assistant": "..."}, ...]
#         :param filter_expr: Milvus 元数据过滤表达式 (如: target_version == 'V6.0')
#         :param pre_retrieved_chunks: 预检索并已重排的文档切片列表（传入时跳过内部检索与重排）
#         """
#         logging.info(f"🚀 开始 QA 链推理，Query: {query} | Filter: {filter_expr}")

#         # 1. 执行混合检索 (传入 filter_expr)
...
#         # 2. 调用 format_prompt 生成包含了“历史对话”与“当轮最新 Context”的模版串
...
#         # 3. 组装 Prompt 并调用 LLM 流式输出
#         for token in self.llm_client.stream_generate(query=query, context=full_prompt):
#             yield token

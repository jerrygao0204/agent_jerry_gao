...
#     def __init__(self):
#         # 内部存储结构: {domain: {package: {tool_name: BaseTool}}}
#         self._hierarchy: Dict[str, Dict[str, Dict[str, BaseTool]]] = {}
#         # 扁平化映射表方便快速查找执行: {tool_name: BaseTool}
...
#         if domain not in self._hierarchy:
#             self._hierarchy[domain] = {}
#         if package not in self._hierarchy[domain]:
#             self._hierarchy[domain][package] = {}

#         self._hierarchy[domain][package][tool.name] = tool
#         self._flat_tools[tool.name] = tool
        
#         logger.info(f"🛠️ [ToolFactory] 注册成功: [{domain} -> {package} -> {tool.name}] (ReadOnly: {tool.is_read_only})")

#     # -------------------------------------------------------------
#     # Level 1 路由支持：获取领域清单 (用于轻量 Router Agent 判定)
#     # -------------------------------------------------------------
#     def get_domains_summary(self) -> List[Dict[str, str]]:
#         """
#         [Level 1] 获取全局领域分类概要。
#         耗时极低、Token 占用极小，专门供第一步路由 Agent 判断使用。
#         """
#         summary = []
#         domain_descriptions = {
#             "rag_knowledge": "处理 FineBI 用户手册、排错指南、FAQ 等非结构化文档检索",
#             "finebi_system": "查询 FineBI 系统仪表板、数据集元数据、权限及定时任务状态",
#             "data_analytics": "对业务数据进行二次统计分析或计算"
#         }
#         for domain in self._hierarchy.keys():
#             summary.append({
#                 "domain": domain,
#                 "description": domain_descriptions.get(domain, "未定义说明的业务领域")
#             })
#         return summary

#     # -------------------------------------------------------------
#     # Level 2 & 3 按需导出：仅加载指定 Domain 或 Package 的工具 Schema
#     # -------------------------------------------------------------
#     def get_tools_schema_by_domains(self, domains: List[str]) -> str:
#         """
#         [Level 2 & 3 按需加载] 仅按命中领域导出工具的 API Schema。
#         避免加载无关领域的工具，极大节省 Context 并提高代码生成准确率。
#         """
#         selected_tools: List[BaseTool] = []
        
#         for dom in domains:
#             if dom in self._hierarchy:
#                 for pkg, tools_dict in self._hierarchy[dom].items():
#                     selected_tools.extend(tools_dict.values())
                    
#         return self._build_tools_description_json(selected_tools)

#     def get_tool(self, name: str) -> Optional[BaseTool]:
#         """依据名称直接获取可执行工具 (用于沙箱安全执行阶段)"""
#         return self._flat_tools.get(name)

#     # -------------------------------------------------------------
#     # 辅助方法：格式化输出 API 文档 JSON
#     # -------------------------------------------------------------
#     def _build_tools_description_json(self, tools: List[BaseTool]) -> str:
#         descriptions = []
#         for tool in tools:
#             args_desc = {}
#             if tool.args_schema:

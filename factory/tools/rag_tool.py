...
#         if self.retriever and hasattr(self.retriever, "hybrid_search"):
#             results = self.retriever.hybrid_search(query=query, top_k=top_k)
#             # 格式化输出供 Agent 消费
#             formatted_chunks = []
#             for idx, item in enumerate(results, 1):
#                 formatted_chunks.append(
#                     f"[{idx}] 文档: {item.get('doc_name', '未知')}\n内容: {item.get('content', '')}"
#                 )
#             return "\n\n".join(formatted_chunks) if formatted_chunks else "未检索到相关文档。"

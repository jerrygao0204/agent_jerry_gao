...
#     """接入 SandboxExecutor 与 MemoryManager 事务控制的强约束 ReAct Agent"""

#     def __init__(self, max_steps: int = 5, sandbox_timeout: int = 2):
#         self.max_steps = max_steps
#         # 1. 引入沙箱隔离器
#         self.sandbox = SandboxExecutor(timeout=sandbox_timeout)
#         # 2. 引入事务型 Memory 管理器
#         self.memory_mgr = MemoryManager(max_messages=20)

#     def run_stream(self, query: str) -> Generator[Dict[str, Any], None, None]:
#         """流式运行 Agent 推理链（带事务控制与沙箱防护）"""

#         # ----------------------------------------------------
#         # 📸 Step A: 开启 Memory 事务快照
#         # ----------------------------------------------------
...
#         # 获取上下文（含历史消息与提取到的实体）
#         context = self.memory_mgr.get_context_for_llm()
#         entities = context.get("entities", {})
#         if entities:
#             yield {
#                 "stage": "entity_memory",
#                 "content": f"🧠 **[Memory Entity]** 提取到上下文实体: `{json.dumps(entities, ensure_ascii=False)}`\n",
#             }
...
#                 thought_msg = f"🧠 **[Thought - Step {step_idx}]**: 分析用户问题与实体上下文，决定调用工具检索解决方案..."
#                 yield {"stage": "thought", "content": thought_msg}

#                 # 模拟提取到的 Action (实际由 LLM 正则解析产生)
...
#                 action_msg = f"🔧 **[Action]**: 尝试执行操作 `{action_payload['tool_name']}`"
#                 yield {"stage": "action", "content": action_msg}

#                 # ----------------------------------------------------
#                 # 🛡️ Step C: 触发 Sandbox 安全检测与隔离执行
#                 # ----------------------------------------------------
#                 if action_payload["tool_name"] == "python_sandbox_executor":
#                     # 送入安全沙箱（进行 AST 静态审查与子进程熔断防护）
...
#                         # 抛出安全异常以触发 Rollback 演示
#                         raise ValueError(
#                             f"安全沙箱检测到高危代码，强制中断: {sandbox_res['error']}"
#                         )
#                     else:
#                         obs_msg = f"👁️ **[Observation - 沙箱结果]**: {sandbox_res['result']} (耗时 {sandbox_res['execution_time']}s)"
#                         yield {"stage": "observation", "content": obs_msg}

#                 else:
#                     # 调用普通知识库工具
#                     tool_inst = tool_factory.get_tool(
#                         action_payload["tool_name"]
#                     )
#                     if tool_inst:
#                         obs_result = tool_inst.run(**action_payload["args"])
#                     else:
#                         obs_result = f"知识库检索成功 (模拟数据): 找到关于 FineBI V6.0 与 MySQL 连接提示..."

#                     obs_msg = f"👁️ **[Observation]** 工具返回结果:\n```text\n{obs_result}\n```\n"
#                     yield {"stage": "observation", "content": obs_msg}

                
#                 # 假设成功推导 Final Answer
#                 final_answer = f"💬 **[Final Answer]**: 根据上下文与知识库，已为您找到 FineBI V6.0 问题的解决方案。"
#                 yield {"stage": "final_answer", "content": final_answer}

#                 # 记录 Assistant 消息
#                 self.memory_mgr.process_assistant_output(final_answer)

#                 # ----------------------------------------------------
#                 # ✅ Step D: 推理成功，提交 Memory 事务
#                 # ----------------------------------------------------
#                 self.memory_mgr.commit()
#                 yield {
#                     "stage": "commit",
#                     "content": "\n✅ **[Memory Transaction]** 事务成功提交 (Commit)，记忆已持久化更新！",
#                 }
#                 return

#         except Exception as e:
#             # ----------------------------------------------------
#             # 🚨 Step E: 出现异常，触发 Memory 事务回滚 (Rollback)
#             # ----------------------------------------------------
#             logger.error(f"ReAct 过程发生错误: {e}")
#             self.memory_mgr.rollback()
#             yield {
#                 "stage": "rollback",
#                 "content": f"\n🚨 **[Memory Transaction]** 捕获异常: *{str(e)}*\n已触发 **Rollback 事务回滚**，对话状态已重置，拒绝污染 Session 上下文！",
#             }
...
#         "请问在 FineBI V6.0 中，怎么解决 MySQL 数据库连接报错？"
#     ):
#         print(step["content"])
